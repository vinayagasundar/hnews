...
class StoriesViewModel(private val storiesActionProcessorHolder: StoriesActionProcessorHolder) :
    ...
    override fun actionFromIntents(intents: StoriesIntent): StoriesAction {
        return when (intents) {
            is StoriesIntent.InitialIntent -> StoriesAction.LoadStoriesAction(true)
            is StoriesIntent.RefreshIntent -> StoriesAction.LoadStoriesAction(intents.forcedUpdate)
        }
    }
